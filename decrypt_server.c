...
int sign(EVP_PKEY *priv,
         uint8_t *msg, size_t msglen,
         uint8_t *sig, size_t *siglen)
{
    EVP_MD_CTX *ctx = EVP_MD_CTX_new();

    EVP_DigestSignInit(ctx,NULL,NULL,NULL,priv);
...
bool verify( EVP_PKEY *pub,
         uint8_t *msg, size_t msglen,
         uint8_t *sig,  int siglen)
{
    EVP_MD_CTX* v=EVP_MD_CTX_new();
    EVP_DigestVerifyInit(v,NULL,NULL,NULL,pub);
    if(EVP_DigestVerify(v, sig, siglen, msg , msglen)<=0)
    {   printf("Auth failed\n");
        return 0;
    }
    EVP_MD_CTX_free(v);
...
int main(){
    ...
    sign(server_priv, transcript, 64, sig, &siglen);   
    send_all(c,sig,siglen);

    uint8_t client_sig[64]; 
    recv_all(c,client_sig,64);
    verify(client_pub, transcript, 64, client_sig, 64  );
...
dest.sin_port = htons(1114);
inet_pton(AF_INET, "127.0.0.1", &dest.sin_addr);

connect(s2,(void*)&dest,sizeof(dest));

uint64_t net_plen = htobe64(plen);

send_all(s2,(uint8_t*)&net_plen,8);
printf("Length---%d\n", plen);
send_all(s2,pt,plen);
printf("Plain Text -- %s\n", pt);
